...
说明：`namcap` 对某些“按架构分离的变量名”（例如 `source_x86_64`）会给出风格性提示（如建议使用 `$CARCH`）。这类在 AUR 多架构包中属于可接受的告警，本项目对该类已知告警做了精确白名单放行。

对于产物包（`*.pkg.tar.*`）的 `namcap` 结果：`E` 会阻止 push，`W` 会被记录到日志/报告但不阻塞（因为二进制上游往往无法做到 0 warning，但我们仍然强制跑 lint 并可追踪结果）。
